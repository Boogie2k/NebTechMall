...
/*  cart.map((items:any)=>console.log(items)) */
 ...
 <Link href={`/phone/${items._id}`}> <img className='h-40 w-45 lg:h-60 lg:w-40 '  src={items.image} alt =''/> <h4 className='text-center font-bold  text-sm'>{items.name} {items.model}</h4>
    </Link> <div className='flex flex-col lg:flex-row justify-between  relative top-5 items-center '><p className='text-red-500 text-xl'>${items.price.toLocaleString()}</p>
  ...
  <svg onClick={()=>{setCart([...cart, {
    
    product:items.name +' ' + items.model, qty:qty, price:items.price, image:items.image,
